...
import { acceptEstimateFromPortal } from "@/app/p/[token]/actions";

/**
 * The customer accepting a QuickBooks estimate: the same signature block as a quote approval.
 *
 ...
 */
export function PortalAcceptance({
  token,
  estimateId,
  consentText,
  smsConsentText,
      ...
      <button
        ...
        onClick={() =>
          startTransition(async () => {
            const res = await acceptEstimateFromPortal(token, estimateId, name, consented, textsOk);
            setResult(res);
            if (res.ok) router.refresh();
          })
        }
      >
        {pending ? "Recording…" : "Accept and sign"}
      </button>
